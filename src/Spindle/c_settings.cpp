...
void Spindle_Controller::c_settings::load_defaults()
{
	...
	Spindle_Controller::c_settings::serializer.values.pP = .05;
	Spindle_Controller::c_settings::serializer.values.pI = .01;
	Spindle_Controller::c_settings::serializer.values.pD = .001;
...
void Spindle_Controller::c_settings::load_settings()
{
	...
	//Assume that if encoder ticks are not set, we do not have a stored configuration at this time.
