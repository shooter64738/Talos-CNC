...
c_settings::settings_t c_settings::settings;

// Method to store startup lines into EEPROM
void c_settings::settings_store_startup_line(uint8_t n, char *line)
{
#ifdef FORCE_BUFFER_SYNC_DURING_EEPROM_WRITE
    c_protocol::protocol_buffer_synchronize(); // A startup line may contain a motion and be executing. 
#endif
    uint8_t *addr = EEPROM_ADDR_STARTUP_BLOCK + n * (LINE_BUFFER_SIZE + 1);
    c_eeprom::memcpy_to_eeprom_with_checksum(addr, line, LINE_BUFFER_SIZE);
}

// Method to store build info into EEPROM
// NOTE: This function can only be called in IDLE state.
void c_settings::settings_store_build_info(char *line)
{
    // Build info can only be stored when state is IDLE.
    c_eeprom::memcpy_to_eeprom_with_checksum(EEPROM_ADDR_BUILD_INFO, line, LINE_BUFFER_SIZE);
}

// Method to store coord data parameters into EEPROM
void c_settings::settings_write_coord_data(uint8_t coord_select, float *coord_data)
{
#ifdef FORCE_BUFFER_SYNC_DURING_EEPROM_WRITE
    c_protocol::protocol_buffer_synchronize();
#endif
    uint8_t *addr = coord_select * (sizeof(float) * N_AXIS + 1) + EEPROM_ADDR_PARAMETERS;
    c_eeprom::memcpy_to_eeprom_with_checksum(addr, coord_data, sizeof(float) * N_AXIS);
}
...
void c_settings::settings_restore(uint8_t restore_flag)
{
...
uint8_t c_settings::settings_read_startup_line(uint8_t n, char *line)
{
    uint8_t *addr = n * (LINE_BUFFER_SIZE + 1) + EEPROM_ADDR_STARTUP_BLOCK;
    if (!(c_eeprom::memcpy_from_eeprom_with_checksum(line, addr, LINE_BUFFER_SIZE)))
    {
...
// Reads startup line from EEPROM. Updated pointed line string data.
uint8_t c_settings::settings_read_build_info(char *line)
{
    if (!(c_eeprom::memcpy_from_eeprom_with_checksum(line, (uint8_t *)EEPROM_ADDR_BUILD_INFO, LINE_BUFFER_SIZE)))
    {
        // Reset line with default value
        line[0] = 0; // Empty line
...
uint8_t c_settings::settings_read_coord_data(uint8_t coord_select, float *coord_data)
{
    uint8_t *addr = coord_select * (sizeof(float) * N_AXIS + 1) + EEPROM_ADDR_PARAMETERS;
    if (!(c_eeprom::memcpy_from_eeprom_with_checksum(coord_data, addr, sizeof(float) * N_AXIS)))
    {
...
uint8_t c_settings::read_global_settings()
{
    ...
    if (version == SETTINGS_VERSION)
    {
        ...
        if (!(c_eeprom::memcpy_from_eeprom_with_checksum(&settings, EEPROM_ADDR_GLOBAL, sizeof(settings_t))))
        {
            return (false);
        }
    }
...
uint8_t c_settings::settings_store_global_setting(uint8_t parameter, float value)
{
    if (value < 0.0)
    {
    ...
    else
    {
        ...
        switch (parameter)
        {
            ...
            case 2:
                ...
                c_stepper::st_generate_step_dir_invert_masks(); // Regenerate step and direction port invert masks.
                break;
            case 3:
            ...
            case 13:
                ...
                c_system::system_flag_wco_change(); // Make sure WCO is immediately updated.
                break;
            case 20:
                if (int_value)
                {
            ...
            case 21:
                ...
                //c_limits::limits_init(); // Re-init to immediately change. NOTE: Nice to have but could be problematic later.
...
uint8_t c_settings::get_step_pin_mask(uint8_t axis_idx)
{
...
uint8_t c_settings::get_direction_pin_mask(uint8_t axis_idx)
{
...
uint8_t c_settings::get_limit_pin_mask(uint8_t axis_idx)
{
