...
class c_settings
{
	...
	static void write_global_settings();
	// Reads selected coordinate data from EEPROM
	static uint8_t settings_read_coord_data(uint8_t coord_select, float *coord_data);

	static uint8_t read_global_settings();
	// Returns the step pin mask according to Grbl's internal axis numbering
	static uint8_t get_step_pin_mask(uint8_t i);

	// Returns the direction pin mask according to Grbl's internal axis numbering
	static uint8_t get_direction_pin_mask(uint8_t i);

	// Returns the limit pin mask according to Grbl's internal axis numbering
	static uint8_t get_limit_pin_mask(uint8_t i);
	protected:
	private:
