...
class c_planner
{
	...
	static void plan_reset(); // Reset all
	static void plan_reset_buffer(); // Reset buffer only.
	...
	static void plan_discard_current_block();

	// Gets the planner block for the special system motion cases. (Parking/Homing)
	static plan_block_t *plan_get_system_motion_block();

	// Gets the current block. Returns NULL if buffer empty
	static plan_block_t *plan_get_current_block();
	...
	static float plan_compute_profile_nominal_speed(plan_block_t *block);

	// Re-calculates buffered motions profile parameters upon a motion-based override change.
	static void plan_update_velocity_profile_parameters();

	// Reset the planner position vector (in steps)
	static void plan_sync_position();

	// Reinitialize plan with a partially completed block
	static void plan_cycle_reinitialize();

	// Returns the number of available blocks are in the planner buffer.
	static uint8_t plan_get_block_buffer_available();

	// Returns the number of active blocks are in the planner buffer.
	// NOTE: Deprecated. Not used unless classic status reports are enabled in config.h
	static uint8_t plan_get_block_buffer_count();

	// Returns the status of the block ring buffer. True, if buffer is full.
	static uint8_t plan_check_full_buffer();

	static void plan_get_planner_mpos(float *target);
