...
{
	Hardware_Abstraction_Layer::Grbl::Coolant::initialize();
...
uint8_t c_coolant::coolant_get_state()
{
	...
	if (bit_isfalse(COOLANT_FLOOD_PORT,(1 << COOLANT_FLOOD_BIT)))
	{
		...
		void c_coolant::coolant_set_state(uint8_t mode)
		{
			if (c_system::sys.abort) return; // Block during abort.

			Hardware_Abstraction_Layer::Grbl::Coolant::set_state( mode);
			c_system::sys.report_ovr_counter = 0; // Set to report change immediately
		}

		// G-code parser entry-point for setting coolant state. Forces a planner buffer sync and bails
		// if an abort or check-mode is active.
		void c_coolant::coolant_sync(uint8_t mode)
		{
			if (c_system::sys.state == STATE_CHECK_MODE) return;

			c_protocol::protocol_buffer_synchronize(); // Ensure coolant turns on when specified in program.
