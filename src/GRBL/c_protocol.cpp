...
c_Serial c_protocol::control_serial;
...
void c_protocol::protocol_main_loop()
{
	...
	for (;;)
	{
		...
		if (c_protocol::control_serial.HasEOL())
		{ // End of line reached
			uint8_t index = 0;
			memset(line,0,LINE_BUFFER_SIZE);
			while (c_protocol::control_serial.Peek() != CR && c_protocol::control_serial.Peek() != NULL)
			{
				line[index] =  toupper(c_protocol::control_serial.Get());
				if (line[index]!=' ')
				index++;
			}
			line[index] = 0;
...
void c_protocol::protocol_execute_realtime()
{
	...
	{
		Hardware_Abstraction_Layer::Serial::send(0, 's');
...
void c_protocol::protocol_exec_rt_system()
{
	...
	if (rt_exec)
	{
		//Hardware_Abstraction_Layer::Serial::send(0, 'x');
	...
	if (rt_exec)
	{
		Hardware_Abstraction_Layer::Serial::send(0, 'u');
	...
	if (rt_exec)
	{
		Hardware_Abstraction_Layer::Serial::send(0, 'v');
