...
uint8_t c_planner::plan_next_block_index(uint8_t block_index)
{
	block_index++;
...
uint8_t c_planner::plan_prev_block_index(uint8_t block_index)
{
	if (block_index == 0)
	{
...
*/
void c_planner::planner_recalculate()
{
	...
	{ // Only two plannable blocks in buffer. Reverse pass complete.
		...
		{
			c_stepper::st_update_plan_block_parameters();
		}
	}
	else
...
void c_planner::plan_reset()
{
	memset(&pl, 0, sizeof(planner_t)); // Clear planner struct
	plan_reset_buffer();
}

void c_planner::plan_reset_buffer()
{
	block_buffer_tail = 0;
...
void c_planner::plan_discard_current_block()
{
	if (block_buffer_head != block_buffer_tail)
	{ // Discard non-empty buffer.
		uint8_t block_index = c_planner::plan_next_block_index(block_buffer_tail);
...
c_planner::plan_block_t *c_planner::plan_get_system_motion_block()
{
	return (&block_buffer[block_buffer_head]);
}

// Returns address of first planner block, if available. Called by various main program functions.
c_planner::plan_block_t *c_planner::plan_get_current_block()
{
	if (block_buffer_head == block_buffer_tail)
	{
		return (NULL);
...
float c_planner::plan_get_exec_block_exit_speed_sqr()
{
...
uint8_t c_planner::plan_check_full_buffer()
{
...
float c_planner::plan_compute_profile_nominal_speed(plan_block_t *block)
{
	float nominal_speed = block->programmed_rate;
	if (block->condition & PL_COND_FLAG_RAPID_MOTION)
	{
		nominal_speed *= (0.01 * c_system::sys.r_override);
	}
	else
	{
		if (!(block->condition & PL_COND_FLAG_NO_FEED_OVERRIDE))
		{
			nominal_speed *= (0.01 * c_system::sys.f_override);
		}
...
void c_planner::plan_compute_profile_parameters(plan_block_t *block, float nominal_speed, float prev_nominal_speed)
{
...
void c_planner::plan_update_velocity_profile_parameters()
{
...
void c_planner::plan_sync_position()
{
	...
	for (idx = 0; idx < N_AXIS; idx++)
	{
		...
		if (idx==X_AXIS)
		{
			pl.position[X_AXIS] = c_system::system_convert_corexy_to_x_axis_steps(c_system::sys_position);
		}
		else if (idx==Y_AXIS)
		{
			pl.position[Y_AXIS] = c_system::system_convert_corexy_to_y_axis_steps(c_system::sys_position);
		}
		else
		{
			pl.position[idx] = c_system::sys_position[idx];
		}
		#else
		pl.position[idx] = c_system::sys_position[idx];
		#endif
...
uint8_t c_planner::plan_get_block_buffer_available()
{
	if (block_buffer_head >= block_buffer_tail)
	{
...
uint8_t c_planner::plan_get_block_buffer_count()
{
	if (block_buffer_head >= block_buffer_tail)
	{
...
void c_planner::plan_cycle_reinitialize()
{
