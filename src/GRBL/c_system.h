...
class c_system
{
	...
	static int32_t sys_position[N_AXIS];      // Real-time machine (aka home) position vector in steps.
	static int32_t sys_probe_position[N_AXIS]; // Last probe position in machine coordinates and steps.

	//static volatile uint8_t sys_probe_state;   // Probing state value.  Used to coordinate the probing cycle with stepper ISR.
	//static volatile uint8_t sys_rt_exec_state;   // Global realtime executor bitflag variable for state management. See EXEC bitmasks.
	//static volatile uint8_t sys_rt_exec_alarm;   // Global realtime executor bitflag variable for setting various alarms.
	//static volatile uint8_t sys_rt_exec_motion_override; // Global realtime executor bitflag variable for motion-based overrides.
	//static volatile uint8_t sys_rt_exec_accessory_override; // Global realtime executor bitflag variable for spindle/coolant overrides.
	static uint8_t sys_probe_state;   // Probing state value.  Used to coordinate the probing cycle with stepper ISR.
	static uint8_t sys_rt_exec_state;   // Global realtime executor bitflag variable for state management. See EXEC bitmasks.
	static uint8_t sys_rt_exec_alarm;   // Global realtime executor bitflag variable for setting various alarms.
	static uint8_t sys_rt_exec_motion_override; // Global realtime executor bitflag variable for motion-based overrides.
	static uint8_t sys_rt_exec_accessory_override; // Global realtime executor bitflag variable for spindle/coolant overrides.
	...
	static void system_flag_wco_change();
	...
	static uint8_t system_check_travel_limits(float *target);

	// Special handlers for setting and clearing Grbl's real-time execution flags.
	static void system_set_exec_state_flag(uint8_t mask);
	static void system_clear_exec_state_flag(uint8_t mask);
	static void system_set_exec_alarm(uint8_t code);
	static void system_clear_exec_alarm();
	static void system_set_exec_motion_override_flag(uint8_t mask);
	static void system_set_exec_accessory_override_flag(uint8_t mask);
	static void system_clear_exec_motion_overrides();
	static void system_clear_exec_accessory_overrides();
	protected:
	private:
