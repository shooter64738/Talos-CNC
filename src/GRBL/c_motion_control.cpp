...
void c_motion_control::mc_line(float *target, c_planner::plan_line_data_t *pl_data, NGC_RS274::NGC_Binary_Block *target_block)
{
    ...
    if (bit_istrue(c_settings::settings.flags, BITFLAG_SOFT_LIMIT_ENABLE))
    {
        ...
        {
            c_limits::limits_soft_check(target);
        }
    }
    ...
    do
    {
        ...
        if (c_planner::plan_check_full_buffer())
        {
...
void c_motion_control::mc_dwell(float seconds)
{
    if (c_system::sys.state == STATE_CHECK_MODE)
    {
        return;
    ...
    utils::delay_sec(seconds, DELAY_MODE_DWELL);
}

// Perform homing cycle to locate and set machine zero. Only '$H' executes this command.
// NOTE: There should be no motions in the buffer and Grbl must be in an idle state before
// executing the homing cycle. This prevents incorrect buffered plans after homing.
void c_motion_control::mc_homing_cycle(uint8_t cycle_mask)
{
    // Check and abort homing cycle, if hard limits are already enabled. Helps prevent problems
    // with machines with limits wired on both ends of travel to one limit pin.
    // TODO: Move the pin-specific LIMIT_PIN call to limits.c as a function.
#ifdef LIMITS_TWO_SWITCHES_ON_AXES
    if (c_limits::limits_get_state())
    {
        mc_reset(); // Issue system reset and ensure spindle and coolant are shutdown.
        c_system::system_set_exec_alarm(EXEC_ALARM_HARD_LIMIT);
        return;
    }
#endif

    //c_limits::limits_disable(); // Disable hard limits pin change register for cycle duration
    ...
    if (cycle_mask)
    {   c_limits::limits_go_home(cycle_mask);} // Perform homing cycle based on mask.
    else
#endif
    {
        // Search to engage all axes limit switches at faster homing seek rate.
        c_limits::limits_go_home(HOMING_CYCLE_0);  // Homing cycle 0
#ifdef HOMING_CYCLE_1
        c_limits::limits_go_home(HOMING_CYCLE_1);  // Homing cycle 1
#endif
#ifdef HOMING_CYCLE_2
        c_limits::limits_go_home(HOMING_CYCLE_2);  // Homing cycle 2
#endif
    }

    c_protocol::protocol_execute_realtime(); // Check for reset and set system abort.
    if (c_system::sys.abort)
    {
        return;
...
uint8_t c_motion_control::mc_probe_cycle(float *target, c_planner::plan_line_data_t *pl_data, uint8_t parser_flags, NGC_RS274::NGC_Binary_Block *target_block)
{
    ...
    c_system::system_set_exec_state_flag(EXEC_CYCLE_START);
    do
    {
        c_protocol::protocol_execute_realtime();
        if (c_system::sys.abort)
    ...
    if (c_system::sys_probe_state == PROBE_ACTIVE)
    {
        ...
        {
            c_system::system_set_exec_alarm(EXEC_ALARM_PROBE_FAIL_CONTACT);
        }
    }
    else
    {
    ...
    c_stepper::st_reset(); // Reset step segment buffer.
    c_planner::plan_reset(); // Reset planner buffer. Zero planner positions. Ensure probing motion is cleared.
    c_planner::plan_sync_position(); // Sync planner position to current machine position.

#ifdef MESSAGE_PROBE_COORDINATES
    // All done! Output the probe position as message.
    c_report::report_probe_parameters();
#endif

    if (c_system::sys.probe_succeeded)
    {
...
void c_motion_control::mc_parking_motion(float *parking_target, c_planner::plan_line_data_t *pl_data)
{
    if (c_system::sys.abort)
    {
        return;
    ...
    if (plan_status)
    {
        ...
        while (c_system::sys.step_control & STEP_CONTROL_EXECUTE_SYS_MOTION);
        c_stepper::st_parking_restore_buffer(); // Restore step segment buffer to normal run state.
    }
    else
    {
        bit_false(c_system::sys.step_control, STEP_CONTROL_EXECUTE_SYS_MOTION);
        c_protocol::protocol_exec_rt_system();
    }
...
void c_motion_control::mc_reset()
{
    // Only this function can set the system reset. Helps prevent multiple kill calls.
    if (bit_isfalse(c_system::sys_rt_exec_state, EXEC_RESET))
    {
        c_system::system_set_exec_state_flag(EXEC_RESET);

        // Kill spindle and coolant.
        c_spindle::spindle_stop();
        c_coolant::coolant_stop();

        // Kill steppers only if in any motion state, i.e. cycle, actively holding, or homing.
        // NOTE: If steppers are kept enabled via the step idle delay setting, this also keeps
        // the steppers enabled by avoiding the go_idle call altogether, unless the motion state is
        // violated, by which, all bets are off.
        if ((c_system::sys.state & (STATE_CYCLE | STATE_HOMING | STATE_JOG)) || (c_system::sys.step_control & (STEP_CONTROL_EXECUTE_HOLD | STEP_CONTROL_EXECUTE_SYS_MOTION)))
        {
            ...
            {
                c_system::system_set_exec_alarm(EXEC_ALARM_ABORT_CYCLE);
            }
            c_stepper::st_go_idle(); // Force kill steppers. Position has likely been lost.
        }
    }
}
