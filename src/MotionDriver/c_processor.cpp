...
void c_processor::check_hardware_faults()
{
	...
	if (Hardware_Abstraction_Layer::MotionCore::Inputs::Driver_Alarms >0)
	{
		...
		c_processor::status_message.system_state = BinaryRecords::e_system_state_record_types::System_Error;
		for (int i=0;i<MACHINE_AXIS_COUNT;i++)
		{
			...
			{
				c_processor::status_message.system_sub_state =  (BinaryRecords::e_system_sub_state_record_types) i;
...
void c_processor::check_sequence_complete()
{
	if (Motion_Core::Hardware::Interpollation::Last_Completed_Sequence != 0 )
	{
		...
		if (Motion_Core::Hardware::Interpollation::Interpolation_Active)
		{
			c_processor::status_message.system_state = BinaryRecords::e_system_state_record_types::Motion_Active;
		}
		else
		{
			c_processor::status_message.system_state = BinaryRecords::e_system_state_record_types::Motion_Idle;
		}
		
		if (!Motion_Core::Settings::StepControl)
		{
		...
		else if (Motion_Core::Settings::StepControl > 0 )
		{
			c_processor::status_message.system_sub_state = BinaryRecords::e_system_sub_state_record_types::Block_Holding;
		}
		...
		c_processor::debug_serial.print_int32((int32_t)c_processor::status_message.system_state);c_processor::debug_serial.Write(CR);
		c_processor::debug_serial.print_string("sub state = ");
		c_processor::debug_serial.print_int32((int32_t)c_processor::status_message.system_sub_state);c_processor::debug_serial.Write(CR);
		c_processor::debug_serial.print_string("num message = ");
		c_processor::debug_serial.print_int32(c_processor::status_message.num_message);c_processor::debug_serial.Write(CR);
		c_processor::debug_serial.print_string("chr message = ");
		c_processor::debug_serial.print_string(c_processor::status_message.chr_message);c_processor::debug_serial.Write(CR);
