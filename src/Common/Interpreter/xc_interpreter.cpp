...
char NGC_interpreter::Line[CYCLE_LINE_LENGTH];
int NGC_interpreter::HasErrors = 0;
c_block NGC_interpreter::local_block;
c_block *NGC_interpreter::stager_block;
bool NGC_interpreter::normalize_distance_units_to_mm = true;
uint16_t ngc_working_g_group = 0;

void NGC_interpreter::initialize()
{
	clear_line();
	NGC_interpreter::local_block = c_block();
}

void NGC_interpreter::clear_line()
{
	memset(NGC_interpreter::Line, 0, CYCLE_LINE_LENGTH * sizeof(char));
}

int NGC_interpreter::process_line(c_block *plan_block)
{
	//NGC_interpreter::local_block is the block that the intepreter uses for work. If an error is found processing the line, this block will be cleared.
	//plan_block, is the block that the local_block will be copied to, if interpretation is successful. 

	NGC_interpreter::local_block.reset(); //<-- Clear EVERYTHING from the local block
	...
	memcpy(NGC_interpreter::local_block.unit_target_positions, plan_block->unit_target_positions, MACHINE_AXIS_COUNT * sizeof(float));
	memcpy(NGC_interpreter::local_block.g_group, plan_block->g_group, COUNT_OF_G_CODE_GROUPS_ARRAY * sizeof(uint16_t));
	memcpy(NGC_interpreter::local_block.m_group, plan_block->m_group, COUNT_OF_M_CODE_GROUPS_ARRAY * sizeof(uint16_t));
	memcpy(&NGC_interpreter::local_block.persisted_values, &plan_block->persisted_values, sizeof(c_block::s_persisted_values));
	memcpy(NGC_interpreter::local_block.block_word_values, plan_block->block_word_values, COUNT_OF_BLOCK_WORDS_ARRAY * sizeof(float));
	memcpy(&NGC_interpreter::local_block.plane_axis, &plan_block->plane_axis, sizeof(c_block::s_plane_axis));
	memcpy(&NGC_interpreter::local_block.arc_values, &plan_block->arc_values, sizeof(c_block::s_arc_values));
	...
	{
		NGC_interpreter::local_block.reset();
	...
	*/
	NGC_interpreter::normalize_distance_units();
	...
	*/
	...
	memcpy(plan_block->g_group, NGC_interpreter::local_block.g_group, COUNT_OF_G_CODE_GROUPS_ARRAY * sizeof(uint16_t));
	memcpy(plan_block->m_group, NGC_interpreter::local_block.m_group, COUNT_OF_M_CODE_GROUPS_ARRAY * sizeof(uint16_t));
	memcpy(&plan_block->persisted_values, &NGC_interpreter::local_block.persisted_values, sizeof(c_block::s_persisted_values));
	memcpy(plan_block->block_word_values, NGC_interpreter::local_block.block_word_values, COUNT_OF_BLOCK_WORDS_ARRAY * sizeof(float));
	memcpy(&plan_block->plane_axis, &NGC_interpreter::local_block.plane_axis, sizeof(c_block::s_plane_axis));
	memcpy(&plan_block->arc_values, &NGC_interpreter::local_block.arc_values, sizeof(c_block::s_arc_values));
	memcpy(&plan_block->word_defined_in_block_A_Z, &NGC_interpreter::local_block.word_defined_in_block_A_Z, sizeof(uint32_t));
...
*/
int NGC_interpreter::error_check_main()
{
	...
	*/
	...
	{
		int gCode = NGC_interpreter::local_block.g_group[i];

		switch (i)
		{
		case NGC_Gcode_Groups::COORDINATE_SYSTEM_SELECTION: //<--G54,G55,G56,G57,G58,G59,G59.1,G59.2,G59.3
		{
			if (!BitTst(NGC_interpreter::local_block.g_code_defined_in_block, NGC_Gcode_Groups::COORDINATE_SYSTEM_SELECTION))
				break;
	...
	> NGC_RS274::G_codes::CANCEL_CUTTER_RADIUS_COMPENSATION)
		...
		case NGC_Gcode_Groups::CUTTER_RADIUS_COMPENSATION: //<--G40,G41,G42
		{
			...
			if (!BitTst(NGC_interpreter::local_block.g_code_defined_in_block, NGC_Gcode_Groups::CUTTER_RADIUS_COMPENSATION))
				break;

			ReturnValue = NGC_interpreter::error_check_cutter_compensation();
			if (ReturnValue != NGC_Interpreter_Errors::OK)
			{
				return ReturnValue;
			}
		...
		case NGC_Gcode_Groups::MOTION: //<--G0,G1,G02,G03,G38.2,G80,G81,G82,G83,G84,G85,G86,G87.G88.G89
		{
			...
			if (!BitTst(NGC_interpreter::local_block.g_code_defined_in_block, NGC_Gcode_Groups::MOTION))
			...
			if (!NGC_interpreter::local_block.any_axis_was_defined())
			...
			if (NGC_interpreter::local_block.block_word_values['F' - 65] == 0 && gCode > 0)
			{
		...
		{
			if (!BitTst(NGC_interpreter::local_block.g_code_defined_in_block, NGC_Gcode_Groups::NON_MODAL))
				break;
		...
		{
			if (!BitTst(NGC_interpreter::local_block.g_code_defined_in_block, NGC_Gcode_Groups::PLANE_SELECTION))
				break;
		...
		{
			if (!BitTst(NGC_interpreter::local_block.g_code_defined_in_block, NGC_Gcode_Groups::TOOL_LENGTH_OFFSET))
				break;
...
int NGC_interpreter::error_check_plane_select()
{
	...
	case NGC_RS274::G_codes::XY_PLANE_SELECTION:
	{
		...
		NGC_interpreter::local_block.plane_axis.normal_axis.name = 'Z';
		NGC_interpreter::local_block.plane_axis.plane_error = NGC_Interpreter_Errors::MISSING_CIRCLE_AXIS_XY;

		NGC_interpreter::local_block.arc_values.horizontal_center.name = 'I';
		NGC_interpreter::local_block.arc_values.vertical_center.name = 'J';
	...
	case NGC_RS274::G_codes::XZ_PLANE_SELECTION:
	{
		...
		NGC_interpreter::local_block.plane_axis.normal_axis.name = 'Y';
		NGC_interpreter::local_block.plane_axis.plane_error = NGC_Interpreter_Errors::MISSING_CIRCLE_AXIS_XZ;

		NGC_interpreter::local_block.arc_values.horizontal_center.name = 'I';
		NGC_interpreter::local_block.arc_values.vertical_center.name = 'K';
	...
	case NGC_RS274::G_codes::YZ_PLANE_SELECTION:
	{
		...
		NGC_interpreter::local_block.plane_axis.vertical_axis.name = 'Z';
		NGC_interpreter::local_block.plane_axis.normal_axis.name = 'X';
		NGC_interpreter::local_block.plane_axis.plane_error = NGC_Interpreter_Errors::MISSING_CIRCLE_AXIS_YZ;

		NGC_interpreter::local_block.arc_values.horizontal_center.name = 'J';
		NGC_interpreter::local_block.arc_values.vertical_center.name = 'K';
		NGC_interpreter::local_block.arc_values.plane_error = NGC_Interpreter_Errors::MISSING_CIRCLE_OFFSET_JK;
	}
	break;
	}
...
void NGC_interpreter::assign_planes()
{
	//To simplify planes, set defined, and assign to the pointed value for this plane
	local_block.plane_axis.horizontal_axis.defined = NGC_interpreter::local_block.get_defined(local_block.plane_axis.horizontal_axis.name);
	local_block.plane_axis.vertical_axis.defined = NGC_interpreter::local_block.get_defined(local_block.plane_axis.vertical_axis.name);
	local_block.plane_axis.normal_axis.defined = NGC_interpreter::local_block.get_defined(local_block.plane_axis.normal_axis.name);
	...
	local_block.arc_values.horizontal_center.defined = NGC_interpreter::local_block.get_defined(local_block.arc_values.horizontal_center.name);
	local_block.arc_values.vertical_center.defined = NGC_interpreter::local_block.get_defined(local_block.arc_values.vertical_center.name);
...
*/
int NGC_interpreter::error_check_arc()
{
	/*The rules :
	...
	*/
	...
	if (!NGC_interpreter::local_block.plane_axis.horizontal_axis.defined && !NGC_interpreter::local_block.plane_axis.vertical_axis.defined)
		return NGC_interpreter::local_block.plane_axis.plane_error;

	//Was radius specified?
	if (NGC_interpreter::local_block.arc_values.R == 0)
	{
		//Must have H or V, but we don't have to have both
		if (!NGC_interpreter::local_block.arc_values.horizontal_center.defined && !NGC_interpreter::local_block.arc_values.vertical_center.defined)
			return NGC_interpreter::local_block.arc_values.plane_error;
	...
	else
	{
		...
		{
			NGC_interpreter::local_block.set_value('I', offsets[0]);
			NGC_interpreter::local_block.set_value('J', offsets[1]);
		}
	}
...
int NGC_interpreter::error_check_center_format_arc(float*offsets)
{
	float target_r = hypot_f(-offsets[0], -offsets[1]);

	// Compute arc radius for mc_arc. Defined from current location to center.
	NGC_interpreter::local_block.set_value('R', hypot_f(offsets[0], offsets[1]));

	// Compute difference between current location and target radii for final error-checks.
	float delta_r = fabs(target_r - NGC_interpreter::local_block.get_value('R'));
	if (delta_r > 0.005)
	{
		if (delta_r > 0.5)
			return NGC_Interpreter_Errors::CENTER_FORMAT_ARC_RADIUS_ERROR_EXCEEDS_005; // [Arc definition error] > 0.5mm

		if (delta_r > (0.001 * NGC_interpreter::local_block.get_value('R')))
...
int NGC_interpreter::error_check_radius_format_arc(uint8_t horizontal_axis, uint8_t vertical_axis
	, uint8_t normal_axis, float*offsets)
{
	//TODO:add back the delat from origin to destination
	// Calculate the change in position along each selected axis
	float x = 0, y = 0;
	//x =  NGC_interpreter::local_block.get_value(c_axis_planner::machine_axis_names[horizontal_axis]) - c_axis_planner::previous_axis_unit_positions[horizontal_axis]; // Delta x between current position and target
	//y = NGC_interpreter::local_block.get_value(c_axis_planner::machine_axis_names[vertical_axis]) - c_axis_planner::previous_axis_unit_positions[vertical_axis];; // Delta y between current position and target

	// First, use h_x2_div_d to compute 4*h^2 to check if it is negative or r is smaller
	// than d. If so, the sqrt of a negative number is complex and error out.
	float h_x2_div_d = 4.0 * NGC_interpreter::square(NGC_interpreter::local_block.get_value('R')) - NGC_interpreter::square(x) - NGC_interpreter::square(y);
	...
	*/
	...
	if (NGC_interpreter::local_block.get_value('R') < 0)
	{
		h_x2_div_d = -h_x2_div_d;
		NGC_interpreter::local_block.set_value('R', -NGC_interpreter::local_block.get_value('R')); // Finished with r. Set to positive for mc_arc
	}
...
int NGC_interpreter::error_check_non_modal()
{
	...
	case NGC_RS274::G_codes::G10_PARAM_WRITE: //<-G10
	{
		...
		*/
		...
		if (!NGC_interpreter::local_block.get_value_defined('L', iAddress))
		{
		...
		case 10:
		{
			...
			if (!NGC_interpreter::local_block.get_value_defined('P', iAddress))
			{
...
int NGC_interpreter::error_check_canned_cycle()
{
	...
	if (BitTst(NGC_interpreter::local_block.word_defined_in_block_A_Z, A_WORD_BIT))
		return NGC_Interpreter_Errors::CAN_CYLCE_ROTATIONAL_AXIS_A_DEFINED;
	if (BitTst(NGC_interpreter::local_block.word_defined_in_block_A_Z, B_WORD_BIT))
		return NGC_Interpreter_Errors::CAN_CYLCE_ROTATIONAL_AXIS_B_DEFINED;
	if (BitTst(NGC_interpreter::local_block.word_defined_in_block_A_Z, C_WORD_BIT))
	...
	if (BitTst(NGC_interpreter::local_block.word_defined_in_block_A_Z, L_WORD_BIT))
	{
		//L was defined, make sure it >1 and integer
		if (NGC_interpreter::local_block.block_word_values['L' - 65] < 2)
		...
		int mantissa = (NGC_interpreter::local_block.block_word_values['L' - 65] - (int)NGC_interpreter::local_block.block_word_values['L' - 65]) * 100;
		if (mantissa > 0)
	...
	if (!NGC_interpreter::local_block.any_linear_axis_was_defined())
...
int NGC_interpreter::error_check_tool_length_offset()
{
	/*
	...
	*/
	...
	//if (!NGC_interpreter::local_block.get_value_defined('P'))
	{
		if (!NGC_interpreter::local_block.get_value_defined('H', iAddress))
		{
...
int NGC_interpreter::error_check_cutter_compensation()
{
	...
	*/
	...
	if ((local_block.g_group[NGC_Gcode_Groups::CUTTER_RADIUS_COMPENSATION] > NGC_RS274::G_codes::CANCEL_CUTTER_RADIUS_COMPENSATION))
	{
		...
		if (!NGC_interpreter::local_block.get_value_defined('P', iAddress))
		{
			...
			if (!NGC_interpreter::local_block.get_value_defined('D', iAddress))
			{
		...
		else
		{
			...
			if (NGC_interpreter::local_block.get_defined('D'))
			{
...
*/
int NGC_interpreter::parse_values()
{
...
float NGC_interpreter::evaluate_address(char Data[])
{
...
*/
int NGC_interpreter::group_word(char Word, float Address)
{
...
*/
int NGC_interpreter::_gWord(float Address)
{
	/*
	|***************************************************************************************|
	...
	*/
	if (BitTst(NGC_interpreter::local_block.g_code_defined_in_block, ngc_working_g_group))
		/*
		...
		to be corrected
		*/
		return NGC_Interpreter_Errors::G_CODE_GROUP_NON_MODAL_ALREADY_SPECIFIED + ngc_working_g_group; //<--Start with group 0 and add the Group to it. Never mind, _working_g_group IS the group number
		...
		*/
	NGC_interpreter::local_block.g_code_defined_in_block =
		BitSet(NGC_interpreter::local_block.g_code_defined_in_block, ngc_working_g_group);
	return NGC_Interpreter_Errors::OK;
}
...
*/
int NGC_interpreter::_mWord(float Address)
{
	/*
	|****************************************************************************************|
	|                              IMPORTANT INFORMATION                                     |
...
*/
int NGC_interpreter::_pWord(char Word, float iAddress)
{
...
*/
int NGC_interpreter::process_word_values(char Word, float iAddress)
{
	/*
	|***************************************************************************************|
	|                              IMPORTANT INFORMATION                                    |
	...
	*/
	...
	if (BitTst(NGC_interpreter::local_block.word_defined_in_block_A_Z, WordNumber))
	...
	*/
	...
	NGC_interpreter::local_block.word_defined_in_block_A_Z = BitSet(NGC_interpreter::local_block.word_defined_in_block_A_Z, WordNumber);
	...
	*/
	...
	NGC_interpreter::local_block.block_word_values[WordNumber] = iAddress;
...
*/
int NGC_interpreter::convert_to_line_index(uint8_t BlockNumber)
{
	...
	//NGC_interpreter::convert_to_line(&c_gcode_buffer::collection[BlockNumber]);
	return 1;
}

int NGC_interpreter::convert_to_line(c_block *local_block)
{
	memset(Line, 0, 256);
...
int NGC_interpreter::normalize_distance_units()
{
	...
	if (!NGC_interpreter::normalize_distance_units_to_mm)
		return NGC_Interpreter_Errors::OK;
	...
	*/
	...
	NGC_interpreter::local_block.block_word_values[A_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[B_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[C_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[I_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[J_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[K_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[Q_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[R_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[U_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[V_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[X_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[Y_WORD_BIT] *= MM_CONVERSION;
	NGC_interpreter::local_block.block_word_values[Z_WORD_BIT] *= MM_CONVERSION;

	NGC_interpreter::local_block.persisted_values.feed_rate *= MM_CONVERSION;
	//TODO:: need a way to determine the units of D/H in the tool table.
	NGC_interpreter::local_block.persisted_values.active_h *= MM_CONVERSION;
	NGC_interpreter::local_block.persisted_values.active_d *= MM_CONVERSION;

	return NGC_Interpreter_Errors::OK;
}

float NGC_interpreter::hypot_f(float x, float y)
{
	return (sqrt(x * x + y * y));
}
float NGC_interpreter::square(float X)
{
	return X * X;
}
//// default constructor
//NGC_interpreter::c_interpreter()
//{
//} //c_interpreter
//
//// default destructor
