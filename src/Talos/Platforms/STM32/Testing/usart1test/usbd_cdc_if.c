...
static int8_t usart1test_Init     (void);
static int8_t usart1test_DeInit   (void);
static int8_t usart1test_Control  (uint8_t cmd, uint8_t* pbuf, uint16_t length);
static int8_t usart1test_Receive  (uint8_t* pbuf, uint32_t *Len);

USBD_CDC_ItfTypeDef USBD_CDC_usart1test_fops = 
{
  usart1test_Init,
  usart1test_DeInit,
  usart1test_Control,
...
/**
  ...
  * @retval Result of the operation: USBD_OK if all operations are OK else USBD_FAIL
  */
static int8_t usart1test_Receive(uint8_t* Buf, uint32_t *Len)
{
