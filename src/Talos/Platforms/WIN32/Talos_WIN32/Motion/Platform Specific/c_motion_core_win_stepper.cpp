...
namespace Hardware_Abstraction_Layer
{
	...
	{
		uint8_t Stepper::step_port_invert_mask;
		uint8_t Stepper::dir_port_invert_mask;
		uint8_t Stepper::step_mask = STEP_MASK;

		std::thread Stepper::timer1_overflow(Stepper::timer1_overflow_thread);
		uint8_t Stepper::_TIMSK1 = 0;


		uint16_t Stepper::set_delay_from_hardware(
	...
	// Enable Stepper Driver Interrupt
		...
		void Stepper::st_go_idle()
		{
			...
			Stepper::_TIMSK1 &= ~(1 << OCIE1A); // Disable Timer1 interrupt
			//TCCR1B = (TCCR1B & ~((1 << CS12) | (1 << CS11))) | (1 << CS10); // Reset clock to no prescaling.
		}

		void Stepper::port_disable(uint8_t inverted)
		{
			if (inverted)
			{
				//STEPPERS_DISABLE_PORT |= (1 << STEPPERS_DISABLE_BIT);
		...
		void Stepper::port_direction(uint8_t directions)
		{
		...
		void Stepper::TCCR1B_set(uint8_t prescaler)
		{
			uint8_t _CS10 = 0;
			uint8_t _CS11 = 1;
			uint8_t _CS12 = 2;
		...
		void Stepper::OCR1A_set(uint8_t delay)
		{
			//OCR1A = delay;
		}
