...
#define spi_cs_low() (PIOA->PIO_CODR |= PIO_PA28)
#define spi_cs_high() (PIOA->PIO_SODR |= PIO_PA28)
...
void power_off (void)
{
	SPI0->SPI_CR = SPI_CR_SPIDIS;				// Disable SPI function //
...
)
{
	...
	extern_system_counters.ms_cnt_dn_timer = wt;
...
)
{
	...
	Talos::Motion::Main_Process::host_serial.print_string("s1\r\n");
	xchg_spi(token);					// Xmit data token //
	if (token != 0xFD)
	{	// Is data token //
		Talos::Motion::Main_Process::host_serial.print_string("s2\r\n");
		xmit_spi_multi(buff, 512);		// Xmit the data block to the MMC //
		Talos::Motion::Main_Process::host_serial.print_string("s3\r\n");
		xchg_spi(0xFF);					// CRC (Dummy) //
		Talos::Motion::Main_Process::host_serial.print_string("s4\r\n");
		xchg_spi(0xFF);
		Talos::Motion::Main_Process::host_serial.print_string("s5\r\n");
		resp = xchg_spi(0xFF);			// Reveive data response //
		if ((resp & 0x1F) != 0x05)		// If not accepted, return with error //
		{
			Talos::Motion::Main_Process::host_serial.print_string("bad resp ");
			Talos::Motion::Main_Process::host_serial.print_int32((resp & 0x1F));
	...
	return 1;
	Talos::Motion::Main_Process::host_serial.print_string("good resp \r\n");
}
#endif
