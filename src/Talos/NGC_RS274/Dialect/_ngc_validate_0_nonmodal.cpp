...
e_parsing_errors NGC_RS274::Dialect::Group0::_L0010(NGC_RS274::Block_View * v_block, uint16_t param_value, uint16_t L_value)
{
	...
	_set_tool_value(v_block, 'X', L_value, Tool_Control::Active_Tool.offset.X, HORIZONTAL_MOTION_AXIS, &updating_tool.offset.X);
	_set_tool_value(v_block, 'Y', L_value, Tool_Control::Active_Tool.offset.Y, VERTICAL_MOTION_AXIS, &updating_tool.offset.Y);
	_set_tool_value(v_block, 'Z', L_value, Tool_Control::Active_Tool.offset.Z, NORMAL_MOTION_AXIS, &updating_tool.offset.Z);
	_set_tool_value(v_block, 'A', L_value, Tool_Control::Active_Tool.offset.A, HORIZONTAL_ROTARY_AXIS, &updating_tool.offset.A);
	_set_tool_value(v_block, 'B', L_value, Tool_Control::Active_Tool.offset.B, VERTICAL_ROTARY_AXIS, &updating_tool.offset.B);
	_set_tool_value(v_block, 'C', L_value, Tool_Control::Active_Tool.offset.C, NORMAL_ROTARY_AXIS, &updating_tool.offset.C);
	_set_tool_value(v_block, 'U', L_value, Tool_Control::Active_Tool.offset.U, HORIZONTAL_INCRIMENTAL_AXIS, &updating_tool.offset.U);
	_set_tool_value(v_block, 'V', L_value, Tool_Control::Active_Tool.offset.V, VERTICAL_INCRIMENTAL_AXIS, &updating_tool.offset.V);
	_set_tool_value(v_block, 'W', L_value, Tool_Control::Active_Tool.offset.W, NORMAL_INCRIMENTAL_AXIS, &updating_tool.offset.W);
...
void NGC_RS274::Dialect::Group0::_set_tool_value
(NGC_RS274::Block_View * v_block, char param_word, uint16_t L_value, float tool_value, uint8_t position_axis_id, float * updating_value)
{
	float _value = 0;

	if (!v_block->get_word_value(param_word, &_value))
	{
		if (L_value == L10_TOOL_TABLE)//L10 tries to calculate the value the offset NEEDS to be, so that the offset will be where we are now. 
		{
			//add our current position and tool offset together. then subtract the value just entered for the offset.
			_value = v_block->active_view_block->target_motion_position[position_axis_id] + tool_value - _value;
		}
